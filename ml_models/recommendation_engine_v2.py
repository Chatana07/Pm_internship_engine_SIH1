"""
AI-based Recommendation Engine for PM Internship Scheme - Version 2
...
class InternshipRecommendationEngine:
    ...
    def _compute_interaction_features(self, candidate_idx, job_indices):
        ...
        for job_idx in job_indices:
            # Make sure job_idx is within bounds
            ...
            skill_jaccard = self._calculate_skill_jaccard(candidate_skills, str(job['Type_of_job']))
            skill_overlap_count = self._calculate_skill_overlap(candidate_skills, str(job['Type_of_job']))
            
            # Location match (simplified)
            location_match_flag = 1 if str(candidate['job_role']).lower() in str(job['location']).lower() else 0
            
            # Experience difference (using encoded values)
            exp_candidate = candidate.get('experience_enc', 0)
            exp_job = job.get('experience_enc', 0)
            experience_diff = exp_candidate - exp_job
            
            # Same sector flag (simplified)
            same_sector_flag = 1 if str(candidate['job_role']).lower() in str(job['Type_of_job']).lower() else 0
    ...
    def _shortlist_jobs(self, candidate_idx, top_n=100):
        ...
        # Get top N most similar jobs (make sure indices are valid)
        valid_indices = np.where(similarity_scores >= 0)[0]  # Get valid indices
        top_indices = valid_indices[np.argsort(similarity_scores[valid_indices])[-top_n:]][::-1]
        
        # Convert to list and ensure all indices are within bounds
        result_indices = [int(idx) for idx in top_indices if idx < len(self.jobs_df)]
    ...
    def train_model(self, sample_size=500):
        ...
        num_candidates = min(sample_size // 10, len(self.candidates_df))
        
        for i in range(num_candidates):
            candidate_idx = i
            shortlisted_jobs = self._shortlist_jobs(candidate_idx, top_n=20)
            
            # Create positive examples (high similarity jobs)
            candidate_similarities = cosine_similarity(
                candidate_tfidf[candidate_idx:candidate_idx+1], 
                job_tfidf[shortlisted_jobs]
            ).flatten()
            
            for j, job_idx in enumerate(shortlisted_jobs):
                if j < len(candidate_similarities):
    ...
    def recommend(self, candidate_idx, top_k=5, use_model=True):
        ...
        if candidate_idx >= len(self.candidates_df):
            raise ValueError(f"Candidate index {candidate_idx} out of range. Max index is {len(self.candidates_df)-1}")
        
        # Shortlist jobs
        shortlisted_jobs = self._shortlist_jobs(candidate_idx, top_n=100)
        
        if not shortlisted_jobs:
            return []  # No jobs shortlisted
        ...
        if not use_model or self.model is None:
            ...
            # Make sure we only use valid job indices
            valid_job_indices = [idx for idx in shortlisted_jobs if idx < len(job_tfidf)]
            if not valid_job_indices:
                return []
                
            similarity_scores = cosine_similarity(
                candidate_tfidf, 
                job_tfidf[valid_job_indices]
            ).flatten()
            scores = similarity_scores
            
            # Map back to original indices
            shortlisted_jobs = valid_job_indices
        ...
        boosted_scores = []
        for i, (job_idx, score) in enumerate(job_scores[:top_k*2]):  # Boost top candidates
...
if __name__ == "__main__":
    ...
    # Get recommendations for the second candidate
    engine.display_recommendations(1, top_k=5)